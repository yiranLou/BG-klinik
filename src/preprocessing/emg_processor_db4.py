...
# EMG输出文件路径
EMG_OUT_FILE = os.path.join(PROCESSED_DIR, 'emg_norm_db4.sto')

################################################################################
# 小波去噪函数
################################################################################

def wavelet_denoise_db4(data, fs=1500):
    """
    使用db4小波基对EMG信号进行去噪并整流
    """
    # 小波分解
    wavelet = 'db4'
    level = 5
...
def normalize_emg(muscle_name, mvic_c3d_path):
    ...
    # 小波去噪 & 整流
    analog_data_mvic = wavelet_denoise_db4(analog_data_mvic, fs=1500)
...
def emgdata_elbow_r(c3d_path, mvic_bi, mvic_tri, mvic_flx, mvic_ex):
    ...
    if sig_biceps is not None:
        sig_biceps = wavelet_denoise_db4(sig_biceps, fs=1500)
    else:
        sig_biceps = np.zeros(1)

    if sig_triceps is not None:
        sig_triceps = wavelet_denoise_db4(sig_triceps, fs=1500)
    else:
        sig_triceps = np.zeros(1)

    if sig_wrist_flex is not None:
        sig_wrist_flex = wavelet_denoise_db4(sig_wrist_flex, fs=1500)
    else:
        sig_wrist_flex = np.zeros(1)

    if sig_wrist_ext is not None:
        sig_wrist_ext = wavelet_denoise_db4(sig_wrist_ext, fs=1500)
    else:
...
def emgdata_elbow_l(c3d_path, mvic_bi, mvic_tri, mvic_flx, mvic_ex):
    ...
    # 小波去噪和整流
    sig_biceps = wavelet_denoise_db4(sig_biceps, fs=1500) if sig_biceps is not None else np.zeros(1)
    sig_triceps = wavelet_denoise_db4(sig_triceps, fs=1500) if sig_triceps is not None else np.zeros(1)
    sig_wrist_flex = wavelet_denoise_db4(sig_wrist_flex, fs=1500) if sig_wrist_flex is not None else np.zeros(1)
    sig_wrist_ext = wavelet_denoise_db4(sig_wrist_ext, fs=1500) if sig_wrist_ext is not None else np.zeros(1)
...
if __name__ == '__main__':
    print("=== 开始 EMG 处理流程 (db4小波) ===")

    # 直接进行 EMG 提取和归一化
    process_emg(side_is_right=True)

    print("=== EMG 处理流程结束 (db4小波) ===")